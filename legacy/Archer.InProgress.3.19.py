...
class Archer_Portal():
    ...
    def edit_box(self):
        ...
        self.edit_record(new_bcode.get(), new_title.get(), new_keyword1.get(), new_keyword2.get(), new_keyword3.get(),
                         new_plocation.get(), new_vlocation.get(), new_description.get("1.0", END), bcode, title,
                         keyword1,
                         keyword2,
                         keyword3, plocation, vlocation, description)) \
            .grid(row=16, column=1, sticky=W)

        self.edit_root.mainloop()

    def edit_record(self, new_bcode, new_title, new_keyword1, new_keyword2, new_keyword3, new_plocation, new_vlocation,
                    new_description, old_bcode, old_title, old_keyword1, old_keyword2, old_keyword3, old_plocation,
                    old_vlocation, old_description):
        # Strip leading/trailing whitespaces or line breaks from values
        old_bcode = old_bcode.strip()
        old_title = old_title.strip()
        old_keyword1 = old_keyword1.strip()
        old_keyword2 = old_keyword2.strip()
        old_keyword3 = old_keyword3.strip()
        old_plocation = old_plocation.strip()
        old_vlocation = old_vlocation.strip()
        ...
        query = '''
                ...
                WHERE Barcode=? AND Title=? AND Keyword1=? AND Keyword2=?
                AND Keyword3=? AND PhysicalLocation=? AND VirtualLocation=? AND Description=?
                '''
        query_commit = ''''''
        parameters = (
            new_bcode, new_title, new_keyword1, new_keyword2, new_keyword3, new_plocation,
            new_vlocation, new_description,
            old_bcode, old_title, old_keyword1, old_keyword2, old_keyword3, old_plocation, old_vlocation,
            old_description
        )
        self.run_query(query, parameters)
        self.edit_root.destroy()
        self.message['text'] = '{} details were changed to {}'.format(old_bcode, new_bcode)
        self.viewing_records()
